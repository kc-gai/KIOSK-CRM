...
async function main() {
    ...
    // Area 생성 및 Region과 연결
    for (const area of areasData) {
        // 해당 Office에 속한 Region들 찾기
        ...
        const createdArea = await prisma.area.create({
            data: {
                ...
                isActive: true,
                // 첫 번째 관련 Region에 연결 (Area는 하나의 Region에만 연결됨)
                regionId: (await prisma.region.findFirst({ where: { code: relatedRegions[0]?.code } }))?.id || ''
            }
        })

        // addressKeywords 업데이트 (해당 Office 관할의 모든 도도부현)
        const allPrefectures = relatedRegions.map(r => r.prefectures).join(',')
        await prisma.area.update({
            where: { id: createdArea.id },
            data: { addressKeywords: allPrefectures }
        })
